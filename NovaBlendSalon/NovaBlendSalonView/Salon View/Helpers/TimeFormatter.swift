...
public enum TimeFormatter {
    public static func convertTo12HourFormat(from time24: Float) -> String? {
        ...
        guard let roundedTime24 = Double(time24.formatted(.number.precision(.fractionLength(2)))) else { return "" }
        
        let hours = Int(roundedTime24)
        
        let diff = (Float(roundedTime24) - Float(hours)) * 100
        let minutes = Int(Double(diff.formatted(.number.precision(.fractionLength(2))))!) // Capture the decimal part as minutes
